...
-- 중간테스트
--1. INSERT, UPDATE, DELETE문 쓸 수 있을 것
--2. 특정 위치에 *표 찍는 것
--3. SELECT로 특정 조건을 만족하는 값 출력
--4. 내장함수  : SUBSTR, REPLACE, ROUND etc.
--5. INNER/OUTER JOIN, 그룹함수, ROWNUM, SUBQUERY가 조합된 것
...
SELECT *
    ...
    INNER JOIN ORDERS O ON S.BOOKID = O.BOOKID
)T
...
SELECT ORDERID, ORDERDATE, NAME, --BOOKID
FROM (
    SELECT *
    FROM (
        SELECT *
...
INNER JOIN ORDERS O ON S.CUSTID = O.CUSTID;
...
SELECT DEPTNO, GRADE--EMPNO, ENAME, SAL, GRADE
FROM EMP E
