...
--GROUP BY S.STU_NO, S.STU_DEPT
--HAVING S.STU_DEPT = '컴퓨터정보'; 
...
FROM STUDENT
GROUP BY STU_NO, STU_NAME, STU_HEIGHT
HAVING STU_HEIGHT > (
...
SELECT S.STU_NAME, S.STU_DEPT, S.STU_HEIGHT, ROUND(AVG_HEIGHT)
FROM STUDENT S
INNER JOIN (
    SELECT STU_DEPT, AVG(STU_HEIGHT) AS AVG_HEIGHT
    FROM STUDENT
    GROUP BY STU_DEPT
)T ON S.STU_DEPT = T.STU_DEPT AND S.STU_HEIGHT > AVG_HEIGHT;
...
--INNER JOIN EMP E3 ON E2.ENAME = E3.ENAME;
...
INNER JOIN (
    ...
    FROM EMP
    GROUP BY DEPTNO
)T ON E.DEPTNO = T.DEPTNO AND E.SAL = MAX_SAL;
...
SELECT P.NAME, P.ID, D.DNAME, PAY + NVL(BONUS,0)  
FROM PROFESSOR P
